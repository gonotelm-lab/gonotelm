...
import (
	...
	markdownsplit "github.com/gonotelm-lab/gonotelm/pkg/eino-ext/chunker/markdown"
...
func NewChunkTransformer(chunkSize, overlapSize int, lenFn LenCalculator) *ChunkTransformer {
	...
	ht, _ := htmlsplit.NewHeaderSplitter(ctx, &htmlsplit.HeaderConfig{
	...
	mt, _ := markdownsplit.NewSplitter(ctx, &markdownsplit.Config{
		ChunkSize:   chunkSize,
		LenFunc:     lenFn,
...
func tryUseEmbeddedPositionMeta(sourceContent string, doc *schema.Document) bool {
	mdByteStart, ok1 := getMetaInt(doc.MetaData, markdownsplit.MetaChunkByteStartKey)
	mdByteEnd, ok2 := getMetaInt(doc.MetaData, markdownsplit.MetaChunkByteEndKey)
	mdRuneStart, ok3 := getMetaInt(doc.MetaData, markdownsplit.MetaChunkRuneStartKey)
	mdRuneEnd, ok4 := getMetaInt(doc.MetaData, markdownsplit.MetaChunkRuneEndKey)
	if !ok1 || !ok2 || !ok3 || !ok4 {
		return false
	}
...
func setChunkPositionMeta(doc *schema.Document, startByte, endByte, startRune, endRune int) {
	...
	doc.MetaData[entity.ChunkMetaPosStartKey] = startRune
	doc.MetaData[entity.ChunkMetaPosEndKey] = endRune
	doc.MetaData[entity.ChunkMetaPosByteStartKey] = startByte
	doc.MetaData[entity.ChunkMetaPosByteEndKey] = endByte
}
