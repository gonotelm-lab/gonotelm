...
type fakeGenerator struct {
	genFn func(ctx context.Context, req *audioschema.Request, opts ...audios.Option) (*audioschema.Response, error)
}

func (f *fakeGenerator) Generate(ctx context.Context, req *audioschema.Request, opts ...audios.Option) (*audioschema.Response, error) {
	if f.genFn != nil {
		return f.genFn(ctx, req, opts...)
	}
	return &audioschema.Response{URL: "http://example.com/a.mp3"}, nil
}
...
func TestTracingGeneratorSpan(t *testing.T) {
	...
	_, err := tracer.Generate(context.Background(), &audioschema.Request{Model: "cosyvoice-v1", Text: "hello"})
	if err != nil {
	...
	sp := spans[0]
	if sp.Name != "gen_ai.text2audio" {
		t.Fatalf("span name: got %q want %q", sp.Name, "gen_ai.text2audio")
	}
	...
	assertSpanAttr(t, sp, "gen_ai.operation.name", "text2audio")
	assertSpanAttr(t, sp, "gen_ai.request.model", "cosyvoice-v1")
...
func TestTracingGeneratorRecordsError(t *testing.T) {
	exporter := newSpanExporter(t)
	tracer := &tracingGenerator{system: "minimax", impl: &fakeGenerator{
		genFn: func(ctx context.Context, req *audioschema.Request, opts ...audios.Option) (*audioschema.Response, error) {
			return nil, context.Canceled
		},
	}}

	if _, err := tracer.Generate(context.Background(), &audioschema.Request{Text: "hi"}); err == nil {
		t.Fatal("want error")
	}
