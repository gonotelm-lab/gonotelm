...
func mustChunkPosition(chunk *schema.Document) (chunkPosition, error) {
	...
	startRaw, ok := chunk.MetaData[entity.ChunkMetaPosStartKey]
	if !ok {
		return chunkPosition{}, fmt.Errorf("chunk missing %s metadata, content=%q", entity.ChunkMetaPosStartKey, chunk.Content)
	}
	endRaw, ok := chunk.MetaData[entity.ChunkMetaPosEndKey]
	if !ok {
		return chunkPosition{}, fmt.Errorf("chunk missing %s metadata, content=%q", entity.ChunkMetaPosEndKey, chunk.Content)
	}
	byteStartRaw, ok := chunk.MetaData[entity.ChunkMetaPosByteStartKey]
	if !ok {
		return chunkPosition{}, fmt.Errorf("chunk missing %s metadata, content=%q", entity.ChunkMetaPosByteStartKey, chunk.Content)
	}
	byteEndRaw, ok := chunk.MetaData[entity.ChunkMetaPosByteEndKey]
	if !ok {
		return chunkPosition{}, fmt.Errorf("chunk missing %s metadata, content=%q", entity.ChunkMetaPosByteEndKey, chunk.Content)
	}
