...
func TestSourceStoreCRUD(t *testing.T) {
	Convey("SourceStore CRUD", t, func() {
		...
		So(got.Abstract, ShouldEqual, source.Abstract)
...
func TestSourceStoreBatchDelete(t *testing.T) {
	Convey("SourceStore BatchDelete should delete target ids only", t, func() {
		...
		err := store.BatchDelete(ctx, []database.Id{src1.Id, src2.Id, src3.Id})
		So(err, ShouldBeNil)

		var deletedCount int64
		err = testDB.WithContext(ctx).
			Model(&schema.Source{}).
			Where("id IN ?", []database.Id{src1.Id, src2.Id, src3.Id}).
...
func TestSourceStoreUpdateAbstract(t *testing.T) {
	Convey("SourceStore UpdateAbstract", t, func() {
		store := testSourceStore
		ctx := t.Context()
		notebookID := createNotebookForSourceTest(t, testDB)
		...
		newUpdatedAt := time.Now().UnixMilli()
		err := store.UpdateAbstract(ctx, &schema.SourceUpdateAbstractParams{
			Id:        source.Id,
		...
		So(err, ShouldBeNil)
		So(got, ShouldNotBeNil)
		So(got.Abstract, ShouldEqual, "abstract-new")
		// ensure unrelated fields are not overwritten.
...
func TestSourceStoreUpsert(t *testing.T) {
	Convey("SourceStore Upsert", t, func() {
		...
		source.NotebookId = database.Id(uuid.NewV7())
...
func TestSourceStoreListByIds(t *testing.T) {
	Convey("SourceStore ListByIds", t, func() {
		...
		rows, err := store.ListByIds(ctx, []database.Id{src1.Id, src2.Id})
		So(err, ShouldBeNil)
		So(len(rows), ShouldEqual, 2)

		idSet := map[database.Id]struct{}{}
...
func TestSourceStoreListByIdsBatches(t *testing.T) {
	Convey("SourceStore ListByIds should work with batches", t, func() {
		...
		rows, err := store.ListByIds(ctx, []database.Id{src1.Id, src2.Id, src3.Id})
		So(err, ShouldBeNil)
...
func TestSourceStoreListByNotebookIdAndIds(t *testing.T) {
	Convey("SourceStore ListByNotebookIdAndIds", t, func() {
		...
		rows, err := store.ListByNotebookIdAndIds(ctx, notebookID, []database.Id{src1.Id, src2.Id, srcOther.Id})
		So(err, ShouldBeNil)
		So(len(rows), ShouldEqual, 2)
...
func createNotebookForSourceTest(t *testing.T, db *gorm.DB) database.Id {
	t.Helper()
